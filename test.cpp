...
{	
	...
	string s;string ref; string new_s;
	string tempstring;
	int vowel_no=0;
...
void train(string s)
{
	...
	{	
		...
		if(ste>20000 && flag == false)
		{
			//begin = sample_count;
			//p.first = start_current;
			flag =true;
		}
		if(ste<20000 && flag == true)
		{
			//end = sample_count - 320;
			flag = false;
			//p.second = start_current;
			calc_ci(s,start_stable);
			max_ste = 0;
			//marker.push_back(p);
		}
		count=0;
...
void calc_ci(string s,long int pos)
{	
	vector<ld> vctr;
	/// Calculating ai's
	int no_of_frame=0;
...
void testing(string s)
{
	...
	s = new_s;
	train(s);/// Now my ci vector has ci of 5frames for each utterance(10)
